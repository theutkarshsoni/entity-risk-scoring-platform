...
**2026-09-16** — `network_access_events` frequency intentionally reverses the autonomy ordering used in `file_access_events`: Fully-autonomous entities generate the fewest network_access events (0-2) despite having the highest file_access volume, while Supervised entities generate the most (0-6) despite the lowest file_access volume.
Why: file_access reflects data-gathering breadth (less oversight → more unchecked internal activity, since a Fully-autonomous entity does the work itself and doesn't need to send results elsewhere). network_access reflects the need to surface findings externally for human review (more supervision → more routing data outward to a human who takes action on it). The two tables model different behavioral axes, not the same "autonomy = more activity" pattern applied twice.

**2026-09-14** — All event-table timestamps are generated as local wall-clock time then converted to timezone-aware UTC via `zoneinfo` before storage, using one shared `generate_timestamp(date, timezone, min_sec, max_sec)` function reused across all four event tables.
Why: real systems store timestamps in UTC always; local time only matters for computing business-hours rules downstream. One shared function (rather than each table implementing its own conversion) meant a naive-vs-aware datetime bug between human and non-human paths was fixed once, not four times.
